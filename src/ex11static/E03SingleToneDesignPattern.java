...
/*
싱글톤 디자인 패턴
: 클래스를 설계하는 디자인 패턴의 하나로써 하나의 인스턴스 즉, 하나의 메모리를 할당한 후
이를 프로그램 전체에서 공유해서 사용하고자 할 때 쓰는 패턴이다.

작성방법
1. 생성자의 접근지정자를 Private으로 선언한다.
2. 그러면 클래스 외부에서는 생성자에 접근할 수 없으므로 new룰 통해 인스턴스 생성이
불가능해진다.
3. 클래스 내부에 멤버변수로 static으로 선언하여 객체를 미리 생성해 놓는다.
4. getInstance()라는 유틸리티 메서드를 통해 참조값을 반환받아 사용한다.
5. 이 방법을 통해 인스턴스를 생성하면 메모리에 딱 하나만 만들어지게 되어 메모리를
절약할 수 있다.
 */
...
class NoSingleTone {
	 ...
	 * 클래스 외부 혹은 다른 패키지에서는 호출할 수 없게되어 new를 통해 인스턴스를
	 * 생성할 수 없기 때문이다. */
	public NoSingleTone() {}
}
//싱글톤 패턴이 적용된 클래스 정의
