...
: 메인쓰레드가 Working쓰레드(개발자가 정의한 쓰레드)를 일컫은 것으로
...
class NormalThread extends Thread
{
	...
	/* 쓰레드 동작중 비정상적으로 종료될 때 InterruptedException이 발생할
	 * 수 있으므로, 예외처리를 해야한다. 여기서는 즉시 처리하지 않고, throws해서
	 * 호출한 위치로 예외객체를 던지기한다. */
...
{
	/* 무한루프로 구성되어있고 별도의 탈출도건을 만들지 않는다.
	 * 3초에 한번씩 Block상태로 들어가게 구현되어있다. */
	@Override
	public void run() {
		while(true) {
			//getName() : 쓰레드명을 반환한다.
			System.out.println("[쓰레드명:%s]Jazz가 흘러요~%n, getName()");
			try {
				sleep(3000);
...
public class Ex03DaemonThread {
	...
	public static void main(String[] args) {
		/* 쓰레들도 구현하지 않는 일반적인 클래스의 작업으로 순차적으로 진행되어
		...
		System.out.println(
				"현재 활성화 된 상태의 쓰레드수"+Thread.activeCount()
				);
		System.out.println(
				"현재 실행중인 쓰레드명:"+Thread.currentThread().getName()
				);
		System.out.println(
				"메인함수의 우선순위:"+Thread.currentThread().getPriority()
				);
	}
