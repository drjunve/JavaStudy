...
없을 때 사용한다.
-만약 해당 클래스가 다른 클래스를 상속해야한다면 Runnable인터페이스를
구현하여 사용한다.
 */
class ShowThread extends Thread
{
	...
	Thread 클래스의 run() 메서드는 오버라이딩 한 것으로 직접
	...
	public void run() {
		...
		for(int i=1 ; i<=100 ; i++) {
			...
			실행의 흐름을 주어진 시간만큼 block상태로 전환시켜 준다.
...
public class Ex01ThreadStart {
	...
	public static void main(String[] args) {
		 ...
		 * 쓰레드가 정상적으로 생성되면 2개의 작업이 병렬적으로 교차하면서
		 * 실행된다. 
		 * */
		st1.start();
		st2.start();
		
		/* run()메스드를 통해 실행하면 단순한 실행만 될 뿐 쓰레드가
