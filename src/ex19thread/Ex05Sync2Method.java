...
class SumMulti2 {
	...
	/*
	쓰레드의 동기화 방법1
	: 동기화 메서드를 사용한다. 메서드전체를 동기화하는 방법이다.
	synchronized 선언으로 인해 정확
	 */
	/*
	...
	*/
	...
	방법이다. 해당프로그램에서는 실행코드가 1중밖에 없으므로
	동기화 방법에 대한 성능차이가 없다. 하지만 실행코드가 많은 경우
	메서드 전체를 동기화하는 것보다 필요한 코드만 동기화하는
...
class MultiAdderThread2 extends Thread {
	 ...
	 * 호출한다. 이 경우 동기화 처리가 되지않아 동시에 접근되는
	 * 경우가 발생하게 되어 잘못된 결과가 출력된다. */
	public void run() {
