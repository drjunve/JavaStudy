...
public class Ex01GenericBasic {
	...
	public static void main(String[] args) {
		 ...
		 * 즉 자료형에는 안전하지만, 구현에는 불편함이 있다. 다른 객체를 저장하기
		 * 위해서는 또 다른 Box클래스를 생성해야한다. */
		
		/* (^↓신텍스 에러, 의도는 오렌지 타입 객체를 호출하는 것이지만, 스트링 타입이다.)
		 * 아래와 같이 Orange가 아닌 객체를 저장하면 즉시 컴파일 에러가 발생하여
		 * 실행할 수 없는 코드가 된다. 즉 자료형에 안전하다고 할 수 있다. */
//		oBox1.store("당도가 20인 오렌지");
		 ...
		 * 다운캐스팅 하는 부분에서는 예외가 발생된다. 즉 런타임 에러가 발생하게
		 * 되므로 실행해보기 전에는 오류를 예측하기 쉽지않은 단점이 있다.*/
		FruitBox fBox2 = new FruitBox("당도가 30인 오렌지");
		Orange orange4 = (Orange)fBox2.pullOut();//런타임 에러 발생1
		orange4.showInfo();
	}
