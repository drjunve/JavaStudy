...
public class MyFriendInfoList {
	...
	public static void main(String[] args) {
		...
		/* 컬렉션은 인스턴스 생성시 크기를 지정할 필요가 없다. 데이터의 갯수에 따라
		자동으로 증가된다. */
...
/* 해당 프로그램에서 기능을 담당하는 클래스로 핸들러 혹은 매니져 클래스라고 부른다. */
	class FriendInfoHandler {
		...
		/* 기존에 인스턴스배열에 저장했던것을 컬렉션으로 변경한다. 컬렉션은 제네릭을 기반으로 하므로
		 * 인스턴스 생성시 저장할 객체의 타입을 결정하게 된다. 여기서는 Friend클래스를
...
//			numOfFriends = 0;
		...
		/*
		연습문제] List컬렉션을 기반으로 친구(고딩/대딩) 정보를 추가하는
		프로그램을 작성하시오. 1차는 추가/출력1/출력2까지 진행합니다.
...
//			System.out.println("addfriend 호출됨");
			...
			if(choice==1) {
				...
				/* High인스턴스를 생성한다. 해당 인스턴스는 자신의 타입이나 부모타입인 Friend에
				 * 저장할 수 있다. */
				...
				/* 기존 객체배열은 정보를 추가할 떄마다 인덱싱을 위해 변수를 ++하는 부분이 필요했지만,
	...
	//		System.out.println("친구정보 입력이 완료되었습니다.");
		...
		public void searchInfo() {
			...
			if(isFind==true) System.out.println("일치하는 정보를 찾았습니다.");
		...
		public void deleteInfo() {
			...
			while(itr.hasNext()) {
//				Friend fr = itr.next();
//				if(itr.next().name.equals(deleteName)) {
//					myFriends.remove(itr.next());
//				}
				Friend fr=itr.next();
