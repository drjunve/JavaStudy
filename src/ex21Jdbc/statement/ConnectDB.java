...
public abstract class ConnectDB {
	...
	public ConnectDB(String id, String pw) {
		try {
			...
			con = DriverManager.getConnection(
					...
					id, pw);
			System.err.println("오라클 DB 연결성공");
		}
	...
	/* 생성자2 : 앞에서 선언한 생성자를 복사하여 선언된 생성자로 고정된 값으로
	 * DB연결을 진행한다. 오버로딩으로 정의되었다. */
	public ConnectDB() {
		try {
			Class.forName("oracle.jdbc.OracleDriver");
			con = DriverManager.getConnection(
	...
	/*
	상속관계에서 오버라이딩을 목적으로 생성한 추상메서드로 하위 클래스에서 각 업무
	(update, select 등)에 따라 재정의 하게 될 것이다.
