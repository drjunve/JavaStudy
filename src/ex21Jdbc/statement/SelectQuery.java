...
public class SelectQuery extends ConnectDB {
	...
	받는다. 결과로 반환된 레코드의 처음부터 끝가지 next()메서드를 통해 확인 후
	반복 인출한다.
	...
	public void execute() {
		try {
			...
			쿼리문 실행시 사용하는 메서드
			 executeUpdate()
				: insert, update, delete와 같이 기존 레코드에 변화가 생기는
				경우에 사용한다. 실행후 적용된 행의 갯수가 int형으로 반환된다.
			executeQuery()
			 	: select 쿼리문을 실행할 때 사용하는 메서드로 레코드에
			 	영향을 미치지않고, 단순히 조회만 하는경우 사용한다. 조회한 레코드를
			 	반환값으로 받아야 하므로 ResultSet객체가 반환타입으로 정의되어있다.
			 */
			...
			while(rs.next()) {
				 ...
				 * 문자형식으로 출력한다. 쿼리문에 별칭을 사용한 경우 그대로
				 * 사용할 수 있다. 즉 컬럼명, 인덱스, 별칭을 통해 컬럼을
				 * 지정할 수 있다. */
