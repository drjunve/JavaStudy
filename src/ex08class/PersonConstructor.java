...
public class PersonConstructor {
	...
	없는 생성자가 컴파일러에 의해 자동으로 삽입된다. 지금까지 우리가 new 클래스면()과 같이
	사용했던 문장이 바로 디폴트 생성자를 호출하여 객체를 생성하는 것이었다.
	
	** 클래스의 필수 구성요소 3 : 멤버변수, 멤버메소, 생성자
	...
	public PersonConstructor(String name) {
		...
		클래스는 자신, 즉 멤버를 가리킨다. 따라서 좌측항은 멤버변수 name, 우측항은 매개변수 name이 된다. */
		this.name = name;
		age = 1;
		addr = "출처불명";
	...
	public PersonConstructor(String name, int age) {
		/* this()는 생성자에서 다른 생성자를 호출할 때 사용한다. 단, 생성자 내에서만 사용할 수 있고
		 일반 멤버메서드에서는 사용할 수 없다. 즉, 아래 코드는 매개변수가 3개인 생성자를 호출한다.*/
		this(name, age, "미상");
	...
	void initialize(String name, int age, String addr) {
		...
		개발자가 임의로 호출할 수 없다. 멤버메서드는 개발자가 원할 때 언제든 호출이 가능하므로 생성자와는
		용도와 특성이 다르다. 따라서 멤버 메서드에서는 생성자를 호출할 수 없다. */
