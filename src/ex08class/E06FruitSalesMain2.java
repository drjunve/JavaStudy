...
/* 첫번재 버전은 각 클래스의 멤버변수가 고정된 초기값으로 선언되어 상태가 다른
 * 인스턴스를 생성할 수 없다는 단점이 있었다.
 * 두번재 버전에서는 초기화 메서드를 추가하여 다양한 초기값을 가진 인스턴스를 생성할
 * 수 있다. */
...
class FruitSeller2{
	 ...
	 * 또한 초기값이 없는 상태로 선언 자체가 불가능하다. 따라서 기존의 상수를 일반
	 * 변수로 변경해야한다. */
	...
	public void showSaleResult() {
		...
		System.out.println("[판매자]팬매수익 : "+myMoney);
	}
}
