...
public class E07RideAndLoad02 {
	...
	public static void main(String[] args) {
		...
		/* 오버라이딩 처리된 메서드이므로 참소변수와 상관없이 항상 자식쪽 메서드가
		 * 호출된다. 메서드 오버라이딩은 자식족에 정의한 메서드가 부모쪽에 정의된
		 * 메서드를 가리게 된다.
		...
		/* 오버라이딩 처리된 메서드이므로 참조변수에 상관없이 자식쪽 메서드가
		 * 호출된다. 자식 객체가 생성될 때 부모쪽의 메서드가 가려지게 되므로
		 * 참조변수에 영향을 전혀 받지 않는다. */
		parent.excecise();
		parent.sleep();
		parent.printParent();
...
//		parent.walk(20);
		...
		//정적 메서드는 오버라이디 되지 않는다. 클래스명으로 호출한다.
