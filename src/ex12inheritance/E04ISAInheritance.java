...
class Computer {
	...
	/* private 접근지정자로 선언된 멤버메서드는 외부 클래스 혹은 상속받은
	...
	/* private으로 선언된 멤버메서드를 외부에서 호출할 수 있게 public으로
...
class NotebookComputer extends Computer {
	...
	public NotebookComputer(String name, int initCharge) {
		 ...
		 * 앞에 다른 문장이 먼저 기술되면 에러가 발생한다. 항상 부모의 생성자를
		 * 최우선으로 호출해야한다.	 */
		super(name);
		//멤버변수 초기화
		battary = initCharge;
	...
	public void movigCal() {
		...
		if(battary<1) {
			...
			/* 메서드에서 아래오 같이 return을 만나서 실행이 중지된다. */ 
			return;
		}
...
/* 테블릿 노트북 : 컴퓨터의 기본 연산기능과, 휴대이용 그리고 전용 펜으로 드로잉 할 수 있는
 * 기능까지 추가된 컴퓨터를 추상화한다. */
...
public class E04ISAInheritance {
	...
	public static void main(String[] args) {
		...
		noteBook.movigCal();
		noteBook.movigCal();
		noteBook.movigCal();//배터리가 1씩 차감되어 여기까지 사용가능
