...
/*
오버라이딩(Overriding)
: 클래스가 상속관계에 있을 떄 하위클래스에서 상위클래스의 멤버메서드와
완전히 동일한 모양으로 메서드를 재정의 하는 것을 말한다.
오버로딩과 다른 점은 매개변수의 갯수 및 타입, 반환타입 등이 완전히
똑같은 형태로 정의한다는 것이다.
...
class Speaker {
	...
	//멤버변수가 private 이므로 외부에서 설정할 수 있도록 setter를 정의한다.
...
class BaseSpeaker extends Speaker {
	...
	/* @Override : 어노테이션이라 부르고, 오버라이딩 된 메서드에 추가하여
...
//		speaker.setBase(40); //에러 발생
		...
		/* 상속을 한 뒤, 부모를 참조변수로 하면 부모 영역 안에있는
		 * 변수, 메소드 등만 접근이 가능하다.
		 * 그런데 예외로, 메소드를 오버라이딩을 한 경우, 참조변수를 부모와 자식 중
		 * 어느쪽을 참조 변수로 하든 상관없이, 항상 오버라이딩 된 메소드로 접근된다. */
	}
