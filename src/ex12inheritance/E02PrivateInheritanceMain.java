...
class Account {
	 ...
	 * 디폴트 생성자는 개발자가 생성자를 정의하지 않았을 때만 컴파일러에 의해 자동으로
	 * 추가된다. */
	...
	//private멤버변수 외부로 반환할 때 사용하는 메서드(getter)
...
class SavingAccount extends Account {
	 ...
	 * (오버라이딩으로 매개변수가 없는 생성자 만들면 super를 지워도 에러발생안함) */
	public SavingAccount(int initVal) {
		/* 만약 아래 문장을 삭제하면 즉시 에러가 발생한다. 삭제하는 경우
		 * super()라는 문장, 즉 매개변수가 없는 디폴트 생성자를 호출하는
		 * 문장이 자동으로 추가되는데, 부모클래스에는 이런 형태의 생성자가 없으므로
		 * 아래와 같이 명시해야한다. */
		super(initVal);
	}
	/* 부모클래스에 정의된 멤버변수 money는 private이므로 자식쪽에서는
	 * 아래와 같이 접근할 수 없다. 보이지 않으므로 에러가 발생한다. 보이지 않으므로
	 * 에러가 발생한다. 따라서 부모클래스에 protected로 선언된 멤버메서드를
	 * 통해 입금처리를 해야한다. */
	public void saveMoney(int money) {
		//super.money += money;
		depositMoney(money);
	}
	/* 잔고를 확인하기 위한 멤버메서드로 부모의 멤버변수인 money에 직접 접근할 수
	 * 없으므로 아래와 같이 getter를 통해 반환받아 출력해야한다. */
...
public class E02PrivateInheritanceMain {
	...
	public static void main(String[] args) {
		...
		/* 부모클래스를 통해 객체를 생성한다. 하지만 해당 인스턴스 변수를 통해서도
		 * 멤버변수 money에 접근할 수 없다. 현재 접근하려는 위치가
		 * 클래스 외부이기 때문이다. 즉, private 멤버는 무조건 클래스 내부에서만
		 * 접근을 허용한다. */
