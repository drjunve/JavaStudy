...
class BusinessMan extends Man {
	...
	public BusinessMan(String name, String company, String position) {
		/* 상속받은 자식클래스는 부모클래스의 생성자를 호출하여 부모 객체가 메모리에
		 * 먼저 생성되도록 해야한다. super()는 부모클래스의 생성자를 호출하는
		 * 역할을 한다. 만약 super()를 기술하지 않으면 컴파일러에 의해 자동으로
		 * 삽입되어 디폴트 생성자가 호출된다. */
	...
	public void tellYourInfo() {
		 ...
		 * 상속받은 자식클래스에서 접근할 수 없다. private멤버는
		 * 무조건 클래스 내부에서만 접근할 수 있다. */
