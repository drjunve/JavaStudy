...
class HighFriend extends Friend {
	...
	/* (오버라이딩2)
	 * 자식 클래스에서 오버라이딩하여 재정의한 메서드로, 고딩친구의 간략한 정보를
	 * 출력한다. 부모 클래스에서는 실행부가 없는 상태로 정의되었다. */
	@Override
	public void showBasicInfo() {
...
class UnivFriend extends Friend {
	...
	//오버라이딩 1,2 역시 HighFriend클래스와 동일한 해턴으로 정의됨
	@Override
	public void showAllData() {
...
public class E10MyFriendInfoBook {
	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			//2. 선택한 번호에 따라 switch문으로 각 메서드를 호출한다.
			switch(choice) {
			case 1: case 2:
...
/* 해당 프로그램에서 기능을 담당하는 클래스로 핸들러 혹은 매니져 클래스라고 부른다. */
	class FriendInfoHandler {
		 ...
		 * 참조할 수 있으므로, 부모타입의 배열에는 자식타입의 인스턴스를
		 * 저장할 수 있다. */
		
		private Friend[] myFriends;
		/* 배열에 저장된 연락처 정보를 카운트하기 위한 멤버변수.
		 * 추가할 때마다 ++연산자로 1씩 증가시킨다. */
...
//			System.out.println("addfriend 호출됨");
			...
			if(choice==1) {
				 ...
				 * 0번 인덱스에 추가한 뒤 1증가시킨다. 이를 위해 '후위증가'하고 있다. */
			...
			/* 더 이상 실행할 문장이 없으므로 해당 메서드는 메모리에서 소멸되고,
		...
		3. 정보 출력시 배열에 입력된 개체 수 만큼 반복하여 각 원소를 통해
			정보를 출력한다.
		4. 출력을 위한 멤버메서드는 상속관계에서 오버라이딩 처리되어 있으므로
			참조변수의 영향을 받지않고 항상 자식클래스에 오버라이딩 된
			메서드가 호출된다.
		5. 즉 저장된 인스턴스는 Friend타입이지만 오버라이딩을 통해 별도의
			형변환이 필요하지 않다. 항상 원하는 정보를 간단히 출력할 수 있게된다.
		 */
		//저장된 연락처 전체정보 출력
		public void showAllData() {
...
//			System.out.println("showSimpleData 호출됨");
			...
			멤버메서드를 정의해야한다.
			정보가 저장되는 배열은 Friend 타입이므로 자식 인스턴스를
...
//			System.out.println("deleteInfo 호출됨");
			 ...
			 * 삭제한 후 빈자리를 채워넣을 때 사용한다. 인덱스는 0부터
			 * 시작이므로 -1을 초기값으로 설정한다. */
			int deleteIndex = -1;
			...
			if(deleteIndex==-1) {
				 ...
				 * 차후 검색이나 삭제시 NullPointerException이
				 * 발생할 수 있다. */
