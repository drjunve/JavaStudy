...
하위클래스에서 재저으이 한 것을 말한다.
이 경우 하위 클래스에서 정의한 메서드가 상위클래스의 메서드를 가리게 되므로,
...
class C extends B {
	 ...
	 * 반환형, 메서드명, 매개변수의 갯수 및 타입이 모두 동일하다. 즉, 똑같은
	 * 메서드이다. */
	@Override
	public void rideMethod() {
...
public class E07RideAndLoad01 {
	...
	public static void main(String[] args) {
		...
		/* 부모타입의 참조변수로 자식 인스턴스를 참조할 수 있으므로 아래 문장은
		 * 모두 성립된다. */
		A ref1 = new C();
		B ref2 = new C();
		C ref3 = new C();
		...
		System.out.println("오버로딩 처리된 메소드(C를 참조변수로)");
		...
		System.out.println("오버로딩 처리된 메소드(B를 참조변수로)");
		/* B타입으로는 접근범위가 B클래스까지로 제한된다. */
		ref2.loadMethod();
		ref2.loadMethod(1);
//		ref2.loadMethod(1.1);
		
		System.out.println("오버로딩 처리된 메소드(A를 참조변수로)");
...
//		ref1.loadMethod(1.1);
		...
		/* 부모 참조변수로 자식 인스턴스를 참조할 수 있다. 좌측은 부모, 우측은
		 * 자식이므로 모두 가능한 문장이다. */
		A refNum1 = new B();
		A refNum2 = new C();
		B refNum3 = new C();
		...
		//A -> C 참조하므로 가능
		A refId1 = new C(); // refId1.xx 형식을 쓸 경우, A와 C의 메소드 모두 호출 가능?
