...
//자식클래스 : DePartent클래스의 기능을 상속하여 확장한다.
public class DeChild extends DeParent {
	//자식에서 확장한 멤버변수
	String stNumber;
	
	//생성자 : super를 통해 부모의 생성자를 먼저 호출한다.
	protected DeChild(String name, int age, String stNumber) {
		super(name, age);
	...
	public void study() {
		 ...
		 * 통해 접근해야한다. */
//		System.out.println(name + "이(가) 공부한다."); //에러발생(name)
		System.out.println(getName() + "이(가) 공부한다.");
	}
	
	/* 오버라이딩 아님 : 부모클래스의 메서드와 접근지정자, 반환타입, 메서드명까지 같지만,
	...
	/* 메서드 오버라이딩의 조건
	 * - 부모클래스에서 선언될 떄와 비교해서 접근범위가 동일하거나 그보다 넓으면 가능하다.
	 ...
	 * 즉 이 경우 오버로딩으로 자식쪽에서 확장한 메서드가 된다. */
