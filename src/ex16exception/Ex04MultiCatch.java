...
Exception 클래스는 모든 예외를 catch 할 수 있으므로 반드시 마지막 블럭에 사용해야한다.
...
public class Ex04MultiCatch {
	...
	public static void main(String[] args) {
		...
		try {
			...
			/*배열의 크기가 3이므로 인덱스는 2까지만 존재한다. 따라서 여기서도 예외가
			발생된다. */
			array[3] = age;
		}
			 ...
			 * 해당 클래스로 모든 예외를 catch 할 수 있기때문이다. */
			System.out.println("예외가 발생하였습니다.");
			System.out.println(e.getMessage());
			e.printStackTrace();
		}
		/* 앞에서 예외가 발생하더라도 예외처리를 했으므로 프로그램은 끝까지 실행된 후
		 * 정상적으로 종료된다. 실행의 흐름이 마지막까지 이어지게 하는 것이
		 * '예외처리'의 목적이다. */
