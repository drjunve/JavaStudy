...
//	}
	...
	toString() : 객체가 가진 데이터를 String으로 반환해준다.
	 print()가 호출될 때 자동으로 호풀되어 출력에 도움을준다.
	*/
	
	@Override
	public String toString() {
		//정수를 String 으로 변경해서 반환한다.
	...
	public boolean equals(Object obj) {
		...
		if(obj instanceof MyClass) {
			 ...
			 * 그렇지 않으면 부모타입으로는 자식 멤버에 접근할 수 없으므로 멤버변수 data에
			 * 대한 비교가 불가능해진다. */
			MyClass mc = (MyClass)obj;
			//멤버변수의 값을 비교하여...
			if(mc.data==this.data) {
				//값이 동일하면 true를 반환한다.
				System.out.println("MyClass-멤버버동일함");
				return true;
			}
			else {
				//다르면 false 를 반환하다.
...
public class E04Object3Equals {
	...
	public static void main(String[] args) {
		...
		/* 만약 오버라이딩 해놓은 equals()를 삭제한 후
		 * 비교하면 다르다는 결과가 나오게 된다. 즉, 오버라이딩이 필요없는
		 * 경우는 기본클래스일 때만 가능하다. */
		...
		if(mc1==mc2) {
			...
			System.out.println("[두 객체를 equale()메소드로 비교]");
