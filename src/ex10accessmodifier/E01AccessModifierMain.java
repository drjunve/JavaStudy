...
//		one.privateMethod();//에러발생
		 ...
		 * 클래스 내부에서는 접근지정자의 영향을 받지 않으므로 해당 메서드를 통해
		 * 접근하는 것이 가능해지게 된다.
		 * ↓아래 public 메서드가 프라이빗 메서드를 호출하고 있다. */
		one.publicMethod();
		
		/* 동일한 패키지에 선언된 클래스이므로 import 없이 인스턴스 생성이
		 * 가능하다. 또한 default 클래스는 동일 패키지내에서 접근할 수 있다. */
		...
		/* 다른 패키지에 선언된 클래스이므로 인스턴스 생성을 위해 import 선언을
		 * 해야한다. 또한 패키지가 다르면 private, default 멤버는
		 * 접근할 수 없다. */
