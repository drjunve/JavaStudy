...
public class E01JunsuType {
	...
	public static void main(String[] args) {
		...
		-즉, 정수형 변수를 사용할 떄는 거의 대부분 int형을 사용하면 된다.
		 */
		...
		*/
		...
		-byte, short 타입은 게임 캐릭터의 움직임이나 음원 파일 등을 표현할 때 주로 사용된다.
		 */
		byte byte1=50, byte2=55;
		//byte + byte = int타입의 결과 출력
		int result2 = byte1 + byte2;
		System.out.println("short1+short2="+result2);
		 ...
		 * 자바 컴파일러는 정수형 상수를 기본적으로 int형으로 간주하기 때문에 아래 정수를
		 메모리에 로드하는 순간 에러가 발생한다. 이때 접미사(L혹은l)을 붙여서 int형의 자료가 아닌
		 long형의 자료임을 컴파일러에게 알려줘야한다. 이와같이 할당을 위해 입력되는 숫자도 자료형을
